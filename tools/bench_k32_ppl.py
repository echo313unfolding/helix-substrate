#!/usr/bin/env python3
"""
WO-K32-PPL-01: k=32 Flat VQ PPL Benchmark
==========================================
Tests whether k=32 (5-bit) VQ holds under 2% PPL on TinyLlama.
If yes, honest product number is 6.4x compression.

k=32 centroids → 5-bit index per weight
FP32 = 32 bits → 32/5 = 6.4x compression ratio
(Plus negligible codebook overhead + small sidecar overhead)

Strategies:
  A. Baseline (FP32, no compression)
  B. k=256 (8-bit) + sidecar — production control
  C. k=32 (5-bit flat) + sidecar — THE TEST
  D. k=64 (6-bit flat) + sidecar — fallback if k=32 fails
  E. Mixed (kurt>5 → k=256, else → k=32) + sidecar — routing rescue

Output: receipts/k32_ppl/
"""
...
def kmeans_1d(data, k=32, max_iters=10, seed=42):
...
def run_strategy(model_base, sf, eval_tokens, name, k_value, per_tensor_kurtosis=None,
                 mixed_mode=False):
    """Encode all tensors with a strategy, swap weights, measure PPL."""
    print(f"\n  [{name}] Encoding 154 tensors (k={k_value})...")
        ...
        for tensor_type in TENSOR_TYPES:
            ...
            if mixed_mode and per_tensor_kurtosis is not None:
                ...
                if kurt > KURTOSIS_THRESHOLD:
                    recon, n_out = encode_vq(weight_np, k=256)
                    n_fallback += 1
                else:
    ...
    if mixed_mode:
        result["n_primary_k"] = n_primary
        result["n_fallback_k256"] = n_fallback
        # Blended compression ratio
        bits_primary = np.log2(k_value)
        total_weights = N_BLOCKS * len(TENSOR_TYPES)
        result["blended_ratio"] = round(
            32.0 / ((n_fallback / total_weights) * 8 + (n_primary / total_weights) * bits_primary),
            2)
...
def main():
    ...
    print("=" * 70)
    print("  WO-K32-PPL-01: k=32 Flat VQ PPL Benchmark")
    print("  Question: Does k=32 (5-bit) hold under 2% PPL?")
    print("  If yes → 6.4x honest product number")
    print("=" * 70)
    ...
    # Strategy C: k=32 (5-bit flat) + sidecar — THE TEST
    r_k32 = run_strategy(model_base, sf, eval_tokens, "k=32 (5-bit)+sc", k_value=32)

    # Strategy D: k=64 (6-bit flat) + sidecar — fallback
    r_k64 = run_strategy(model_base, sf, eval_tokens, "k=64 (6-bit)+sc", k_value=64)

    # Strategy E: Mixed (kurt>5 → k=256, else → k=32) + sidecar
    r_mixed = run_strategy(model_base, sf, eval_tokens, "Mixed (kurt>5→256, else→32)+sc",
                           k_value=32, per_tensor_kurtosis=per_tensor_kurt, mixed_mode=True)
    ...
    strategies = [
        ...
        ("k=32 (5-bit)+sc", r_k32, "6.4x"),
        ("k=64 (6-bit)+sc", r_k64, "5.3x"),
        ("Mixed k32/k256+sc", r_mixed, f"{r_mixed.get('blended_ratio', '?')}x"),
    ]

    print(f"  {'Strategy':<25} {'PPL':>8} {'delta%':>8} {'Mean cos':>10} {'Min cos':>10} {'Ratio':>7}")
    ...
    # Key question answer
    k32_delta = 100 * (r_k32["ppl"] - ppl_base) / ppl_base
    print(f"\n  === KEY ANSWER ===")
    if abs(k32_delta) < 2.0:
        print(f"  k=32 PASSES at {k32_delta:+.2f}% -> honest product number is 6.4x")
    else:
        k64_delta = 100 * (r_k64["ppl"] - ppl_base) / ppl_base
        if abs(k64_delta) < 2.0:
            print(f"  k=32 FAILS at {k32_delta:+.2f}% but k=64 PASSES at {k64_delta:+.2f}%")
            print(f"  -> honest product number is 5.3x")
        else:
            print(f"  k=32 FAILS ({k32_delta:+.2f}%), k=64 FAILS ({k64_delta:+.2f}%)")
            print(f"  -> ceiling is 4.0x (k=256), competitive story is quality + universality")

    mixed_delta = 100 * (r_mixed["ppl"] - ppl_base) / ppl_base
    if abs(mixed_delta) < 2.0 and abs(k32_delta) >= 2.0:
        print(f"  Mixed routing rescues k=32: {mixed_delta:+.2f}% at {r_mixed.get('blended_ratio', '?')}x")

    print(f"\n  Wall: {wall:.0f}s, CPU: {cpu:.0f}s")

    # Receipt
    receipt_dir = Path(__file__).parent.parent / "receipts" / "k32_ppl"
    receipt_dir.mkdir(parents=True, exist_ok=True)

    receipt = {
        "work_order": "WO-K32-PPL-01",
        "question": "Does k=32 (5-bit) flat VQ hold under 2% PPL on TinyLlama?",
        ...
        "strategies": {
            ...
            "k32_5bit_sidecar": r_k32,
            "k64_6bit_sidecar": r_k64,
            "mixed_k32_k256_sidecar": r_mixed,
        },
        "per_tensor_kurtosis": {k: round(v, 4) for k, v in per_tensor_kurt.items()},
        "verdicts": {},
        "cost": {
    ...
    for name, r, ratio in strategies:
        key = name.replace(" ", "_").replace("/", "_").replace("(", "").replace(")", "").replace("+", "_").replace(">", "gt").replace("=", "eq").replace(",", "")
        delta_pct = 100 * (r["ppl"] - ppl_base) / ppl_base
    ...
    receipt_path = receipt_dir / f"k32_ppl_{ts}.json"
