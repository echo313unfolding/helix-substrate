#!/usr/bin/env python3
"""
WO-K64-CROSS-QWEN-01: k=64 PPL Cross-Validation on Qwen2.5-Coder-1.5B
========================================================================
Tests whether k=64 (6-bit) VQ holds under 2% PPL on Qwen2.5-Coder-1.5B.
Cross-model validation: k=64 already proven on TinyLlama (WO-K32-PPL-01).
...
def run_strategy(model_base, sf, eval_tokens, name, k_value):
    ...
    n_total = N_BLOCKS * len(TENSOR_TYPES)
    print(f"\n  [{name}] Encoding {n_total} tensors (k={k_value})...")
    ...
    for block_idx in range(N_BLOCKS):
        for tensor_type in TENSOR_TYPES:
            ...
            weight_np = sf.get_tensor(hf_name).float().numpy()
...
def main():
    ...
    print("=" * 70)
    print("  WO-K64-CROSS-QWEN-01: k=64 Cross-Validation on Qwen2.5-Coder-1.5B")
    print(f"  Auto-detected: {N_BLOCKS} blocks, {N_BLOCKS * len(TENSOR_TYPES)} tensors")
    print("  Question: Does k=64 (6-bit) hold under 2% PPL on Qwen?")
    print("=" * 70)
    ...
    # Use PyTorch framework — Qwen stores weights in bfloat16 which numpy can't handle
    ...
    # Results table
    print(f"\n{'=' * 70}")
    print("  RESULTS — WO-K64-CROSS-QWEN-01")
    print(f"{'=' * 70}")
    ...
    if abs(k64_delta) < 2.0:
        print(f"  k=64 PASSES on Qwen-1.5B at {k64_delta:+.2f}% -> cross-model validated")
    else:
        print(f"  k=64 FAILS on Qwen-1.5B at {k64_delta:+.2f}% -> single-model only")
    ...
    receipt = {
        "work_order": "WO-K64-CROSS-QWEN-01",
        "question": "Does k=64 (6-bit) flat VQ hold under 2% PPL on Qwen2.5-Coder-1.5B?",
        "model": "Qwen2.5-Coder-1.5B-Instruct",
        "n_blocks": N_BLOCKS,
        "n_tensors": N_BLOCKS * len(TENSOR_TYPES),
    ...
    receipt_path = receipt_dir / f"k64_qwen_1.5b_{ts}.json"
