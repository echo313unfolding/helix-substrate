#!/usr/bin/env python3
"""
Compress Qwen2.5-3B-Instruct to CDNA v3 / HelixLinear format.
...
Output:
  ~/models/qwen2.5-3b-instruct/cdnav3/   (28 blocks × 7 tensors = 252 HelixLinear)

Usage:
  python3 tools/compress_qwen3b.py
"""
...
def main():
    ...
    print("=" * 70)
    print("  Compress Qwen2.5-3B-Instruct → CDNA v3")
    print("=" * 70)
    ...
    # Write receipt
    receipt = {
        "work_order": "WO-QWEN3B-COMPRESS-01",
        "question": "Does Qwen2.5-3B-Instruct compress through CDNA v3?",
    ...
    receipts_dir = Path(__file__).parent.parent / "receipts" / "qwen3b_compress"
    receipts_dir.mkdir(parents=True, exist_ok=True)
    ts = time.strftime("%Y%m%dT%H%M%S")
    receipt_path = receipts_dir / f"qwen3b_compress_{ts}.json"
