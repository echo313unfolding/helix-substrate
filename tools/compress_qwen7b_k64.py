#!/usr/bin/env python3
"""
Compress Qwen2.5-7B-Instruct to CDNA v3 with k=64 codebook.

Same pipeline as compress_qwen7b.py but with 64 centroids instead of 256.
...
Output:
  ~/models/qwen2.5-7b-instruct/cdnav3_k64/   (28 blocks × 7 tensors = 196 HelixLinear)
...
Usage:
  python3 tools/compress_qwen7b_k64.py
"""
...
def main():
    ...
    print("=" * 70)
    print(f"  Compress Qwen2.5-7B-Instruct → CDNA v3 (k={K_VALUE})")
    print("=" * 70)
    ...
    receipt = {
        "work_order": "WO-QWEN7B-K64-COMPRESS-01",
        "question": "Does Qwen2.5-7B-Instruct compress through CDNA v3 at k=64?",
    ...
    receipts_dir = Path(__file__).parent.parent / "receipts" / "qwen7b_k64_compress"
    receipts_dir.mkdir(parents=True, exist_ok=True)
    ts = time.strftime("%Y%m%dT%H%M%S")
    receipt_path = receipts_dir / f"qwen7b_k64_compress_{ts}.json"
